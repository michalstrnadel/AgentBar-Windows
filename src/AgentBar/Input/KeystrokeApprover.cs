...
/// guaranteed (one window can host many tabs). Windows has no Accessibility gate like
/// macOS, but SendInput cannot reach an elevated window from a non-elevated app.
public static class KeystrokeApprover
{
    public static void Approve(Session session, ushort[] keys)
    {
        ...
        // Give the window time to come forward before typing into it (mirrors macOS 0.7s).
